...
async def test_order_by_operation(db_url):
    ...
    results = await AggProduct.where(AggProduct.id >= 0).order_by(AggProduct.price).all()
    assert [r.name for r in results] == ["A", "M", "Z"]
    
    # Sort by name descending
    results = await AggProduct.where(AggProduct.id >= 0).order_by(AggProduct.name, direction="desc").all()
    assert [r.name for r in results] == ["Z", "M", "A"]
    
    # Sort by category (asc), then price (desc)
    results = await AggProduct.where(AggProduct.id >= 0).order_by(AggProduct.category).order_by(AggProduct.price, direction="desc").all()
    assert [r.name for r in results] == ["Z", "M", "A"]
