...
async def test_in_helper(db_url):
    ...
    # Use .in_()
    results = await SearchableUser.where(SearchableUser.name.in_(["user1", "user3"])).all()
    assert len(results) == 2
    assert {r.name for r in results} == {"user1", "user3"}
    
    # Verify << still works
    results_legacy = await SearchableUser.where(SearchableUser.name << ["user1", "user3"]).all()
