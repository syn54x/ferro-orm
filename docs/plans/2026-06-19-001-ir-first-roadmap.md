...
- `Sub-issues:` [#146](https://github.com/syn54x/ferro-orm/issues/146) _(dialect enums; **done**)_, [#153](https://github.com/syn54x/ferro-orm/issues/153) _(create-path unification; **done**, merged via #157)_, [#154](https://github.com/syn54x/ferro-orm/issues/154) _(datetime→timestamptz coarseness; **done**, merged via #167)_, [#155](https://github.com/syn54x/ferro-orm/issues/155) _(py3.14 deferred annotations; **done**)_, [#158](https://github.com/syn54x/ferro-orm/issues/158) _(single db_check check-renderer; **done**)_, [#162](https://github.com/syn54x/ferro-orm/issues/162) _(typed save/update value bind — drop the model_dump_json envelope; closes #160; **done**, merged via #166)_, [#165](https://github.com/syn54x/ferro-orm/issues/165) _(auto-migrate BLOB-read-as-text false positive; **done**, merged via #168)_ — **epic #145 complete, 7/7**
...
- [x] Unify the three dialect enums (`SqlDialect`/`BackendKind`,
...
- [x] Surface the real PEP 649 deferred-annotation eval error instead of swallowing
      it: `_resolve_deferred_annotations` now calls `add_note` with the model name
      and re-raises; the previously misleading "non-annotated attribute" path is
      eliminated; behaviour-preserving for every valid model (0 swallow-branch hits
      across the full suite + examples) ([#155](https://github.com/syn54x/ferro-orm/issues/155)).
- [x] Route `Model.save()`/`create()`/`Query.update()`/`bulk_create()` value
      marshaling through the typed codec bind path (drop the `model_dump_json` /
      `to_json` string envelope): row values cross the FFI as a native per-column
      map with `bytes` preserved; a Rust `BindInput { Json, Bytes }` binds `bytes`
      directly to `SeaValue::Bytes` and delegates everything else to the unchanged
      `schema_bind_expr` (parity by construction). Non-UTF-8 binary now persists
      byte-for-byte (SQLite BLOB / Postgres bytea); closes the user-facing bug #160
      ([#162](https://github.com/syn54x/ferro-orm/issues/162); merged via #166).
- [x] Fix the auto-migrate false-positive that reads a freshly-created `BLOB`
      column back as `text` (spurious "declared text but model expects blob"
      warning on every connect for a `bytes` field; discovered while triaging #162):
      two arms in the single-source `information_schema_to_db_type_token` /
      `db_type_token_to_canonical` normalize a live SQLite `blob` to the canonical
      `blob` token so it round-trips through both planner consumers; genuine
      `text`⇄`blob` diffs still warn; Postgres unaffected
      ([#165](https://github.com/syn54x/ferro-orm/issues/165); merged via #168).
- [x] _Read-only Rust duplication sweep: closed out with the epic. The sub-issues
      above (#146, #153, #158, #162, #165) were its output. One known residual
      duplication surfaced by #165's review — the twin `sqlite_type_class` /
      `sqlite_declared_type` in `src/migrate.rs` vs `ferro-ddl-lowering` — is
      re-homed to Phase 9 / #108, which deletes the legacy planner (and its copies)
      outright rather than syncing them._

**Exit gate**
- [x] Backend matrix green; all cleanups behavior-preserving (no DDL/runtime change).
...
- `2026-06-30` - **Phase 8.6 #155 PEP 649 deferred-annotation fix** landed on `feat/ir-p8.6-155-deferred-annotations` (PR targets integration `feat/ir-p8.6-cleanups`). The swallowed PEP 649 deferred-annotation eval error in `_resolve_deferred_annotations` is replaced by `value_error.add_note("... model '<namespace __qualname__>' ...")` + `raise value_error`; the previously misleading "non-annotated attribute" error path is eliminated. Reframing finding: option (B) "make failing models work" was dropped — `FerroField` has no `default` parameter, so the repro used invalid API and was not a genuine PEP 649 eval failure. Behaviour-preserving for every valid model: 0 swallow-branch hits across the full suite (684 passed, 33 skipped, 1 xfailed on 3.14) and examples. Migration impact `none` — internal metaclass only; the change turns a misleading `PydanticUserError` ("non-annotated attribute") into the real eval error with a model-naming note; no API or behaviour change for any working model. **#145 is now 4/6 sub-issues complete** (#146, #153, #155, #158 done; #154, #162 remain).
- `2026-07-01` - **Phase 8.6 #162 typed save/update value bind** merged to integration `feat/ir-p8.6-cleanups` (PR #166). `Model.save()`/`create()`/`Query.update()`/`bulk_create()` no longer serialize row values to a JSON string (`model_dump_json` / `to_json` / `json.dumps`); each channel now sends a native per-column value map across the FFI with `bytes` preserved, and a Rust `BindInput { Json(serde_json::Value), Bytes(Vec<u8>) }` binds `bytes` directly to `SeaValue::Bytes` while delegating everything else to the unchanged `codec::schema_bind_expr` (parity by construction — `codec.rs` untouched). Non-UTF-8 binary now persists and round-trips byte-for-byte (SQLite BLOB / Postgres bytea) instead of raising `PydanticSerializationError`; **closes the user-facing bug #160**. Extraction happens synchronously (GIL held) before `future_into_py` since `Bound<PyDict>` is not `Send` (mirrors `raw_execute`). Follow-ups filed: [#164](https://github.com/syn54x/ferro-orm/issues/164) (unify the write-path value model on a bytes-native type, retiring the `BindInput` two-arm wrapper) and [#165](https://github.com/syn54x/ferro-orm/issues/165) (auto-migrate BLOB-read-as-`text` false positive, discovered during triage). Migration impact `none` — public API unchanged; the only behaviour change is that binary persists where it previously raised. **#145 is now 5/7 sub-issues complete** (#146, #153, #155, #158, #162 done; #154, #165 remain).
- `2026-07-01` - **Phase 8.6 #154 datetime→timestamptz coarseness** merged to integration `feat/ir-p8.6-cleanups` (PR #167). **Reframed from the filed "low-pri coarse mapping → spurious warning" to a data-safety fix once reproduced on Postgres:** a plain `datetime` model field pointed at an externally-created plain `timestamp` column caused `connect(migrate_updates=True)` to **silently execute** `ALTER COLUMN … TYPE timestamptz USING …::timestamptz` (no warning — the "use Alembic" warning is SQLite-only), reinterpreting every stored value under the connection's session timezone (e.g. a naive-UTC `09:00` read as `09:00-05` = `14:00Z`). Root cause: Python's `datetime` annotation carries no naive/aware distinction, so `canonical_from_parts` maps `datetime`→`TimestampTz` unconditionally (`crates/ferro-ddl-lowering/src/lib.rs`). Fix keeps drift **detection** unchanged and changes only the **remediation**: a pure predicate `is_timestamp_tz_conversion` + a single-sourced `timestamp_tz_conversion_warning` (both in `ferro-ddl-lowering`) make **both** emitters — the IR `emit_alter_column_type` (`ferro-migrate`) and the legacy `plan_existing_column` (`src/migrate.rs`) — **warn-and-skip** the `timestamp`⇄`timestamptz` conversion instead of executing it, leaving the column untouched; the warning names both remediations (`db_type="timestamp"` to keep it naive — already worked and now the only drift-free path — or Alembic with an explicit source timezone to convert). Single-sourcing the warning keeps the IR and legacy planners byte-identical, verified under the shadow comparator **enabled + strict** (`FERRO_SHADOW_RUNTIME=1 FERRO_SHADOW_RUNTIME_STRICT=1`): PG gate 3/3, **0 mismatch**. Every other type change (`int`→`bigint`, …) still auto-runs; Ferro-managed `timestamptz` columns and SQLite (which merges the two types) are unaffected. Migration impact **Postgres-only, safety-improving** — a previously silent data-reinterpreting rewrite becomes a loud, actionable warning that touches nothing; no public API change. **#145 is now 6/7 sub-issues complete** (#146, #153, #154, #155, #158, #162 done; only #165 remains).
- `2026-07-02` - **Phase 8.6 #165 BLOB introspection false positive** merged to integration `feat/ir-p8.6-cleanups` (PR #168). A model with a `bytes` field creates a SQLite `BLOB` column, yet auto-migrate warned on **every** connect that the column was "declared `text` in the database but the model expects `blob`" (with a misleading "use Alembic" instruction) — a pure false positive; the model and DB agree (mirror image of #154, which was Postgres-only). Root cause: the shared introspection normalizer `information_schema_to_db_type_token` (`crates/ferro-ddl-lowering/src/lib.rs`) had **no arm for the live `blob` spelling**, so a live SQLite BLOB fell through to the catch-all `_ => "text"`; that phantom `text` drove the warning in **both** planner paths (IR `schema_columns_storage_drift` and legacy `plan_existing_column` → `sqlite_type_class`). Fix: **two arms in the single-source `lib.rs`** — `information_schema_to_db_type_token` gains `"blob" => "blob"` and `db_type_token_to_canonical` gains `"blob" => Some(Blob)`. Token `"blob"` (not the canonical `"bytea"`) is deliberate: it is the only token that round-trips through **both** consumers — `db_type_token_to_canonical("blob") == Some(Blob)` (IR path) **and** `sqlite_type_class("blob")` classes it like the model's declared blob (legacy path; `sqlite_type_class("bytea")` would be `Numeric`). No emitter/planner changes. Genuine `text`⇄`blob` diffs still warn (both directions); Postgres (live `bytea`) and all other types unaffected. Verified: crate 17/17 (incl. a cross-consumer round-trip guard + drift regression), SQLite+Postgres CI matrix green, and IR-vs-legacy parity under the shadow comparator **enabled + strict** (`FERRO_SHADOW_RUNTIME=1 FERRO_SHADOW_RUNTIME_STRICT=1`) with **0 mismatch**. The final review surfaced one residual duplication (twin `sqlite_type_class`/`sqlite_declared_type` in `src/migrate.rs` vs `ferro-ddl-lowering`) — re-homed to Phase 9 / #108, which deletes the legacy planner outright. Migration impact `none` for data/schema — SQLite never `ALTER`s column types, so no SQL was ever emitted for the case; the only user-visible change is that the spurious warning is gone (one incidental correctness bonus: an explicit `db_type="blob"` override now resolves to `Blob` on both backends). **#145 is now 7/7 sub-issues complete — Epic #145 closed and Phase 8.6 wrapped** (#146, #153, #154, #155, #158, #162, #165 all done). Next: integrate `feat/ir-p8.6-cleanups` upward and begin **Phase 9** (shim removal — #107/#108/#109/#110; #108 deletes the legacy migrate planner + shadow comparator, retiring the duplications 8.6 chose not to hand-sync).
