...
| R5 | Remove the cutover-orphaned create-path emitter (`build_create_table_sqls` + `shadow_compare_create_table_sqls` + `append_composite_*` + their tests). The deprecated arms STAY (legacy planner) — removal re-homed to Phase 9 / #108 | #153 re-scope (Option A) |
| R6 | `schema_json_to_schema_ir` STAYS as the legacy-vs-IR parity fixture until Phase 9; its + the arms' deprecation notes re-pointed at Phase 9 | #153 re-scope (Option A) |
...
The golden test asserts the **`CREATE TABLE` statement is byte-identical** (column order, inline FK, inline `UNIQUE` all live inside it and all matter). Post-create statements (`CREATE [UNIQUE] INDEX`, PG `db_check` `ALTER`) are compared as a **sorted set**, not an ordered list. This is not merely a convenience: exact statement-order parity is **infeasible from the IR** — the compiler sorts `indexes`/`uniques` by name and discards the user's declaration order, so the legacy column-then-composite order cannot be reconstructed for multiple same-kind composites. The statements are independent and order-immaterial (every column exists before any post-create artifact runs), so a set comparison is sound, and the shared emitter stays on its natural sorted-by-name ordering (`standalone_indexes`). `test_cross_emitter_parity.py` (Alembic empty-diff) independently guarantees no schema-level difference.
...
### Task 5: Remove the orphaned create-path emitter; re-home the deprecation to Phase 9

> **Scope correction (discovered during execution — Option A, user-approved).** The
> deprecated `("string", Some(...))` arms and `schema_json_to_schema_ir` are NOT only the
> create path's — they are still required by the **legacy migrate planner**
> `plan_table_migration_legacy` (retained for shadow comparison until **Phase 9**):
> `plan_table_migration_legacy` (`migrate.rs:427`) → `build_column_plan` (`migrate.rs:452`)
> → `canonical_column_type` (`schema.rs:221`) → `canonical_from_parts("string", Some(...))`,
> with `.unwrap_or(Varchar)` swallowing a miss. Removing the arms now would make the legacy
> planner silently resolve `datetime`/`date`/`uuid`/`bytes` → `varchar`, diverge from the IR
> planner, and break the IR-vs-legacy parity tests + production shadow compare
> (`shadow_compare_migration_plan`, reached in `internal_migrate` `migrate.rs:902`). So #153
> removes only the create-path emitter the Task 3 cutover **orphaned**; arm + `schema_json_to_schema_ir`
> removal moves to **Phase 9 / #108** alongside the legacy-planner removal.

**Files:**
- Modify: `src/schema.rs` — DELETE the cutover-orphaned create emitter: `build_create_table_sqls` (:453), `shadow_compare_create_table_sqls` (:495), `append_composite_unique_index_sqls` (:145), `append_composite_index_sqls` (:277), and the Rust tests that call `build_create_table_sqls`. cfg(test)-gate `composite_unique_index_name` (:136) and `composite_index_name` (:189).
- Modify: `crates/ferro-ddl-lowering/src/lib.rs` — UPDATE (do NOT remove) the DEPRECATED comment above the `("string", Some(...))` arms to point removal at Phase 9 / #108.
- Modify: `src/migrate.rs` — UPDATE (do NOT remove) the `schema_json_to_schema_ir` doc comment: retained as the legacy-vs-IR parity fixture until Phase 9.

**KEEP — do NOT delete (still used):** `build_column_plan`, `canonical_column_type`, `property_json_type_and_format` (legacy planner, `migrate.rs:452`); `order_schemas_for_creation` (`internal_migrate`, `migrate.rs:890`); the five deprecated arms + `schema_json_to_schema_ir`; `postgres_enum_type_name_for_column` (pre-existing dead in `schema_bind.rs`, unrelated to #153 — leave it).

**Interfaces:** none produced; removes orphaned dead code + adjusts deprecation notes.

- [ ] **Step 1 — Grep gate.** Confirm the deletion set is genuinely orphaned:

```bash
grep -rn 'build_create_table_sqls\|shadow_compare_create_table_sqls\|append_composite_unique_index_sqls\|append_composite_index_sqls' src/ --include='*.rs'
```

  The only `build_create_table_sqls` references must be inside `shadow_compare_create_table_sqls` (also deleted) and `#[cfg(test)]` tests (also deleted). Any production caller means the Task 3 cutover missed a site — fix that first.

- [ ] **Step 2 — Delete the orphaned emitter + its tests.** Remove `build_create_table_sqls`, `shadow_compare_create_table_sqls`, `append_composite_unique_index_sqls`, `append_composite_index_sqls`, and the Rust tests that call `build_create_table_sqls`. **KEEP** the `canonical_column_type` tests (`schema.rs:992-1006`) — they exercise the surviving shared lowering.

- [ ] **Step 3 — cfg(test)-gate the surviving name helpers.** `composite_unique_index_name` and `composite_index_name` are now used only by the `#[cfg(test)]` `schema_json_to_schema_ir`. Add `#[cfg(test)]` to each, with a one-line comment: retained for the Phase-9 legacy-parity fixture; the production create emitter's composite naming lives in `ferro-ddl-lowering`.

- [ ] **Step 4 — Re-home the deprecation notes.** In `canonical_from_parts`, update the DEPRECATED comment block: the `("string", Some(...))` arms are now reached ONLY by `plan_table_migration_legacy`; removal is gated on the Phase 9 legacy-planner removal (#108), not the create path. Update `schema_json_to_schema_ir`'s doc comment to the same effect.

- [ ] **Step 5 — Run; expect a warning-free build (for the cutover cluster) + green matrix:**

```
cargo build --no-default-features --features testing   # no dead_code warnings for build_create_table_sqls/shadow_compare/append_composite_*
...
```
  ...
  (`postgres_enum_type_name_for_column` may still warn — pre-existing, out of scope; leave it.)

- [ ] **Step 6 — Commit:** `git commit -m "refactor(schema): remove the orphaned JSON create-table emitter (#153)"`

**Verification:** the create-path emitter cluster is gone; no NEW dead_code warnings from the cutover; the legacy planner + its IR-vs-legacy parity tests + shadow compare stay green; full matrix green.
...
**Files:**
- Modify: `docs/plans/2026-06-19-001-ir-first-roadmap.md` (Phase 8.6 deliverable check + progress-log entry + Phase 9 deliverable for the re-homed arm/shim removal)
- Modify: `docs/plans/ir-first-migration-guide.md` (record migration impact `none` with justification)
- Modify: the spec `docs/brainstorms/2026-06-29-ir-p8.6-153-create-path-ir-requirements.md` if any "are removed" claims remain (sync to Option A)

- [ ] **Step 1 — Roadmap sync:** tick the #153 deliverable under Phase 8.6 (create-path unification), add a dated progress-log entry. Record the **scope correction**: the deprecated-arm + `schema_json_to_schema_ir` removal is re-homed to **Phase 9 / #108** (legacy-planner removal) — add it to the Phase 9 deliverable list. Keep epic #145 references consistent.
- [ ] **Step 2 — Issue sync (#153 + #108):** comment on / edit **#153** to record that it delivers the create-path unification and that arm/shim removal moved to Phase 9 (with the legacy-planner rationale); ensure **#108** (Phase 9) tracks the re-homed removal. Per traceability rule, reflect the scope change in both the roadmap and the issues in this session.
- [ ] **Step 3 — Migration guide:** add the #153 row — migration impact `none` (runtime DDL byte-identical; internal-only AddTable SQLite-FK fix), per the documentation policy.
- [ ] **Step 4 — Project board:** confirm #153 on Project #7 with `Status`; keep the native sub-issue link under #145.
- [ ] **Step 5 — Commit:** `git commit -m "docs(ir-p8.6): sync roadmap + migration guide for #153 (arm/shim removal -> Phase 9)"`
- [ ] **Step 6 — PR:** open against `feat/ir-p8.6-cleanups` with `Closes #153` and an exit-steps checklist (AGENTS.md I-9). The PR body must state the re-scope (create-path unification delivered; arm/shim removal Phase 9). Validate the commit range with `cz check` before pushing.

**Verification:** `uv run cz check --rev-range <base>..HEAD` passes; roadmap, #153, #108, and the board read consistently (single source of execution truth).

---

## Self-Review

- **Spec coverage:** R1→T3; R2→T2; R3→T2(golden)+T3(matrix); R4→T1; R5/R6→T5 (re-scoped: orphan-cleanup + deprecation re-home, arm/shim removal → Phase 9 #108); R7→T4; R8→T6. Spec §4.5 (test helper) → T3 Step 4. Spec §Parity details → T2 (KTD-2).
