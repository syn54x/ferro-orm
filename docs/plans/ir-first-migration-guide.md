...
| [#153](https://github.com/syn54x/ferro-orm/issues/153) | Runtime CREATE TABLE path now consumes the Python-compiled SchemaIR via the shared `ferro-migrate` `render_create_table` emitter (inline FKs, `binary` logical type added, fail-loud on unknown types); cutover-orphaned JSON create emitter removed | none | No action — runtime DDL is byte-identical; internal-only change | Scope correction: removal of deprecated `canonical_from_parts` `("string", Some(...))` arms and `#[cfg(test)]` `schema_json_to_schema_ir` re-homed to Phase 9 / #108 (required by `plan_table_migration_legacy` until then) |
| [#146](https://github.com/syn54x/ferro-orm/issues/146) | Three parallel dialect enums (`SqlDialect`/`BackendKind`, `ferro-ddl-lowering::Dialect`, `ferro-migrate::BackendDialect`) unified into one canonical `ferro_ddl_lowering::Dialect`; three translator helpers (`sql_dialect_to_lowering`, `backend_dialect`, `lowering_dialect`) deleted; `from_url` relocated to free function `dialect_from_url` | none | No action — `Dialect` and `dialect_from_url` are internal Rust symbols; runtime DDL and behavior unchanged | Pure type-identity refactor; zero behavior change; Python API unaffected |
| [#158](https://github.com/syn54x/ferro-orm/issues/158) | `SchemaCheck` is now structured `{name, column, values}` (values are pre-rendered SQL literal tokens); one `render_db_check` in `ferro_ddl_lowering` serves both the runtime CREATE and `ferro-migrate` ALTER paths; `render_check_expression` (positional re-quoting from #153) deleted; ALTER path reconciled to quoted form (now matches CREATE) with a SQLite elision warning; Alembic mirrors just the body in Python | none | No action — internal architecture only; runtime CREATE DDL byte-identical; ALTER/Alembic converge to the canonical quoted form; SQLite ALTER gains a warning instead of silently eliding |
| [#155](https://github.com/syn54x/ferro-orm/issues/155) | PEP 649 deferred-annotation eval error in `ModelMetaclass._resolve_deferred_annotations` is no longer swallowed; `add_note` attaches the model name and the real error is re-raised; the misleading "non-annotated attribute" path is eliminated | none | No action — internal metaclass change only; a previously swallowed misleading error now surfaces clearly; no API or behaviour change for any valid model |
...
| [#171](https://github.com/syn54x/ferro-orm/issues/171) | `Query.update()` / `Query.delete()` raise `ValueError` when the query carries `limit()` or `offset()` (previously the pagination was silently ignored and every matching row was mutated); mutating QueryIR payloads structurally omit `limit`/`offset` | breaking | Remove the `limit()`/`offset()` call from mutating queries; to mutate a bounded subset, fetch primary keys first and mutate by primary-key set (`where(lambda m: m.id.in_(ids))`) | Enforced both Python-side (`builder._mutating_query_def`) and by the payload contract (`tests/test_static_contracts.py`); `first()`-then-`delete()` on the same query still works |
| [#173](https://github.com/syn54x/ferro-orm/issues/173) | `create()` is a plain INSERT (previously compiled to `INSERT ... ON CONFLICT (pk) DO UPDATE`); a duplicate primary key or unique value raises `UniqueViolationError` and leaves the existing row untouched | breaking | Callers relying on the clobber move to `Model.upsert(...)`; callers that never hit conflicts are unaffected | Anti-clobber covered by `tests/test_save_semantics.py` on both backends |
| [#174](https://github.com/syn54x/ferro-orm/issues/174) | `save()` tracks persistence state explicitly: transient instances INSERT, persisted instances UPDATE by primary key (a 0-row UPDATE raises `ModelDoesNotExist`); upsert is a named surface — `Model.upsert(**fields)` / `save(on_conflict="update")` (conflict target: primary key only) | breaking | Replace save-as-upsert usage with `Model.upsert(...)` or `save(on_conflict="update")`; audit code that saves instances whose rows may be deleted underneath (now raises instead of re-inserting) | `model_copy()` copies persistence state (saving a copy UPDATEs the same row); `delete()` returns the instance to transient; published guide: `docs/pages/howto/migrating-to-v0-14-0.md`. The epic sketched this error as `StaleObjectError`; it shipped as `ModelDoesNotExist` |
