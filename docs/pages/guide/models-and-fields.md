...
Json-family fields — `dict`, `list` (any element type, including `list[SomeNestedModel]`), and nested Pydantic models — store as JSON documents. On PostgreSQL the default is a plain `json` column; opt into **JSONB** (binary storage, GIN-indexable, containment operators) per column with `db_type="jsonb"`:
...
=== "Assignment"
    ...
    class Transaction(Model):
        id: int | None = Field(default=None, primary_key=True)
        raw_payload: dict[str, Any] = Field(db_type="jsonb")
        line_items: list[Payment] = Field(db_type="jsonb")
        audit_note: dict[str, str] = Field(db_type="json")  # explicit plain json
    ```
...
=== "Annotated"
    ...
    class Transaction(Model):
        id: Annotated[int | None, Field(default=None, primary_key=True)]
        raw_payload: Annotated[dict[str, Any], Field(db_type="jsonb")]
        line_items: Annotated[list[Payment], Field(db_type="jsonb")]
        audit_note: Annotated[dict[str, str], Field(db_type="json")]  # explicit plain json
    ```
...
**Round-trip contract is value equality, not byte fidelity.** PostgreSQL's `jsonb` does not preserve dict key order (keys come back sorted), duplicate keys, or whitespace — that's inherent to the storage type, not Ferro. Values compare equal with Python `==` (dict equality ignores order), but if you iterate keys and need insertion order — or need the stored text byte-for-byte — keep the default `json` storage.

**Migrating between `json` and `jsonb` is a declaration edit.** With [`migrate_updates`](migrations.md) enabled, adding `db_type="jsonb"` to an existing json column emits exactly one `ALTER TABLE ... TYPE jsonb USING ...` on PostgreSQL (existing values survive the cast); removing the opt-in emits the mirror `ALTER` back to `json`. The [Alembic bridge](migrations.md) renders the same DDL. On SQLite the same edit is a no-op.
