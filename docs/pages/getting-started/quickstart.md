...
- `lambda post: post.published == True` is a lambda predicate — the only predicate style `where()` accepts. Name the parameter after the row type in lowercase singular (`user` for `User`, `post` for `Post`). See [Queries](../guide/queries.md#predicate-style) for the full operator surface and how column names are validated at build time.
